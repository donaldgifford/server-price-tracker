...
- [Findings](#findings)
  ...
  - [5. Second-pass findings (2026-05-15 follow-up)](#5-second-pass-findings-2026-05-15-follow-up)
    - [Architecture (second-pass)](#architecture-second-pass)
    - [5.A1 internal/store/postgres.go imports internal/metrics — Important](#5a1-internalstorepostgresgo-imports-internalmetrics--important)
    - [5.A2 internal/engine/engine.go holds concrete config.AlertsConfig — Critical](#5a2-internalengineenginego-holds-concrete-configalertsconfig--critical)
    - [5.A3 NewPostgresStore silently ignores DatabaseConfig.PoolSize — Critical](#5a3-newpostgresstore-silently-ignores-databaseconfigpoolsize--critical)
    - [5.A4 Duplicate stripJSONFences in pkg/extract and pkg/judge — Important](#5a4-duplicate-stripjsonfences-in-pkgextract-and-pkgjudge--important)
    - [5.A5 internal/engine/score.go exports package-level functions that are unused — Important](#5a5-internalenginescorego-exports-package-level-functions-that-are-unused--important)
    - [5.A6 internal/engine/scheduler.go imports pkg/observability/langfuse only for WithSessionID — Important](#5a6-internalengineschedulergo-imports-pkgobservabilitylangfuse-only-for-withsessionid--important)
    - [5.A7 QuotaHandler accepts concrete *ebay.RateLimiter — Nice-to-have](#5a7-quotahandler-accepts-concrete-ebayratelimiter--nice-to-have)
    - [5.A8 Four handlers still accept the full store.Store interface — Nice-to-have](#5a8-four-handlers-still-accept-the-full-storestore-interface--nice-to-have)
    - [Style (second-pass)](#style-second-pass)
    - [5.S1 Magic verdict thresholds in judge worker — Important](#5s1-magic-verdict-thresholds-in-judge-worker--important)
    - [5.S2 Magic MaxTokens: 256 in judge generate request — Important](#5s2-magic-maxtokens-256-in-judge-generate-request--important)
    - [5.S3 Duplicate const batchSize = 200 — Important](#5s3-duplicate-const-batchsize--200--important)
    - [5.S4 Missing var _ LLMBackend = (*X)(nil) compliance assertions — Important](#5s4-missing-var-_-llmbackend--xnil-compliance-assertions--important)
    - [5.S5 Unreachable panic in OTel noop meter registration — Nice-to-have](#5s5-unreachable-panic-in-otel-noop-meter-registration--nice-to-have)
    - [5.S6 Hard-coded HTTP timeout in NewAnthropicBackend — Nice-to-have](#5s6-hard-coded-http-timeout-in-newanthropicbackend--nice-to-have)
    - [5.S7 Test helpers missing t.Helper() — Nice-to-have](#5s7-test-helpers-missing-thelper--nice-to-have)
    - [5.S8 Scheduler lock TTLs are bare literals — Nice-to-have](#5s8-scheduler-lock-ttls-are-bare-literals--nice-to-have)
    - [Performance (second-pass)](#performance-second-pass)
    - [5.P1 GetAlertDetail makes two round-trips when one would suffice — Critical](#5p1-getalertdetail-makes-two-round-trips-when-one-would-suffice--critical)
    - [5.P2 Missing covering index for queryHasRecentAlert — Critical](#5p2-missing-covering-index-for-queryhasrecentalert--critical)
    - [5.P3 Missing covering index for queryHasSuccessfulNotification — Important](#5p3-missing-covering-index-for-queryhassuccessfulnotification--important)
    - [5.P4 DiscordNotifier uses http.DefaultClient — Important](#5p4-discordnotifier-uses-httpdefaultclient--important)
    - [5.P5 todayUTCMidnight recomputed per iteration in judge budget check — Important](#5p5-todayutcmidnight-recomputed-per-iteration-in-judge-budget-check--important)
    - [5.P6 Missing composite partial indices for unextracted / unscored listings — Nice-to-have](#5p6-missing-composite-partial-indices-for-unextracted--unscored-listings--nice-to-have)
    - [5.P7 Synchronous Langfuse Score loop on alert-dismiss HTTP handler — Nice-to-have](#5p7-synchronous-langfuse-score-loop-on-alert-dismiss-http-handler--nice-to-have)
    - [5.18 Findings investigated and rejected](#518-findings-investigated-and-rejected)
